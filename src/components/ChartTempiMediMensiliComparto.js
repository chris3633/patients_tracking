...
export function ChartTempiMediMensiliComparto(props) {
    const options = {
        indexAxis: 'y',
        elements: {
            bar: {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Grafico tempi medi mensili comparto (in ore)',
            },
        },
    };
...
function conversioneOre(stringa){
    ...
    props.statistiche_comparto && Object.keys(props.statistiche_comparto[0]['stat_mensile']).map((mese, index) => (
    obj = {},
    console.log(mese),
    valori_mesi = [],

    props.statistiche_comparto[0]['stat_mensile'][mese] && labels.map((mese) => (

        props.statistiche_comparto[0]['stat_mensile'][mese] ?
            valori_mesi.push(conversioneOre( props.statistiche_comparto[0]['stat_mensile'][mese]))
