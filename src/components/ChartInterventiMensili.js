...
export function ChartInterventiMensili(props) {
    const options = {
        indexAxis: 'y',
        elements: {
            bar: {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Grafico interventi effettuati su base mensile',
            },
        },
    };
...
function conversioneOre(stringa){
    ...
    props.statistiche_interventi && Object.keys(props.statistiche_interventi[0]['stat_mensili']).map((mese, index) => (
    obj = {},
    valori_mesi = [],

    props.statistiche_interventi[0]['stat_mensili'][mese] && labels.map((mese) => (

        props.statistiche_interventi[0]['stat_mensili'][mese] ?
            valori_mesi.push(props.statistiche_interventi[0]['stat_mensili'][mese])
