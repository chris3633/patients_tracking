...
export function ChartTempiMediGiornalieriStanza(props) {
    const options = {
        indexAxis: 'y',
        elements: {
            bar: {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Grafico tempi medi giornalieri per stanza (in ore)',
            },
        },
    };
...
function conversioneOre(stringa){
    ...
    props.statistiche_stanze[0][stanza] && labels.map((giorno) => (
        ...
        props.statistiche_stanze[0][stanza]['statistiche_giornaliere'][giorno] ?
            valori_mesi.push(conversioneOre(props.statistiche_stanze[0][stanza]['statistiche_giornaliere'][giorno]))
